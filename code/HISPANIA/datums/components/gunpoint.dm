...
/datum/component/gunpoint
	...
	var/obj/item/left_hand
	var/obj/item/right_hand
...
/datum/component/gunpoint/Initialize(mob/living/targ, obj/item/gun/wep)
	...
	playsound(shooter, 'sound/weapons/targeton.ogg', 85, 1, -1)
...
/datum/component/gunpoint/proc/trigger_reaction()
	...
	if(!weapon.can_shoot() || !weapon.can_trigger_gun(shooter) || (weapon.weapon_weight == WEAPON_HEAVY && shooter.get_inactive_hand()))
	...
	if(CLUMSY in shooter.mutations)
		return
...
/datum/component/gunpoint/proc/cancel()
	...
	playsound(shooter, 'sound/weapons/targetoff.ogg', 75, 1, -1)
	qdel(src)

/datum/component/gunpoint/proc/flinch(attacker, damage, damagetype, def_zone)
	var/mob/living/shooter = parent

	var/flinch_chance = 88
	var/gun_hand = left_hand

	if(shooter.is_in_active_hand(right_hand))
		gun_hand = right_hand

	if((def_zone == BODY_ZONE_L_ARM && gun_hand == left_hand) || (def_zone == BODY_ZONE_R_ARM && gun_hand == right_hand) || GUNPOINT_MULT_STAGE_3)
		flinch_chance = 100
